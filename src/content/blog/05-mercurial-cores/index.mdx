...
Google is running a massive-scale data-analysis pipeline. But one day
they noticed that it starts giving incorrect results. Investigation results, suggests that it 
...
This somehow causes a small subset of machines to repeatedly produce incorrect results. 

Deeper investigation revealed that the real underlying issue is on the hardware, specifically from manufacturing 
defects in the CPU chips. The defect causes one core in a multi-core chip to silently produce 
incorrect computation results. They call this silent errors as "corrupt execution errors" or CEEs.
The rate of incidence of which is higher than what the typical software engineers expected. 
The authors don't disclosed the exact numbers due to business reasons, but they mentioned that it's 
"a few mercurial cores per several thousand machines".

The behavior of the defect is quite complex and interesting in my view. It happens non-deterministically, repeatedly, 
intermittently, and it gets worse over time. The authors also mentioned that external factors such as temperature, voltage, 
and frequency can influence the rate of incidence as well; They found that some CEE rates are very sensitive to frequency changes, 
...
Pretty much all computer systems that aren't specialized to handle this level of fault-tolerance (which is the majority) 
operate under the assumption that the possibility of CPUs producing incorrect results are 
so minuscule that it's fine to assume that it never happens.
But this discovery challenges that assumption.

Randomly incorrect computation results in a commercial-grade CPU could have serious implications. 
Few that I can think of are:
1. Data rendered permanently inaccessible because of incorrect computation results during the encryption process.
2. Loss of money in a financial system.
3. Medical system malfunction causing fatal impact on patients.
4. Self-driving car accidents.
5. Military device/weapon malfunction causing fatal consequences.

And a lot more.

And these things has actually happened at Google:
...
1. Applying the [End-to-End Argument](https://en.wikipedia.org/wiki/End-to-end_principle).
